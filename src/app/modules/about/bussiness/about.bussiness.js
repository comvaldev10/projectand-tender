...
const edit_about = async (req, response) => {
  try {
    if (req.user.role_id == 2) {
      let obj = {
        about_name: req?.body?.about_name ? req?.body?.about_name : '',
        about_select_sector: req?.body?.about_select_sector ? req?.body?.about_select_sector : '',
        about_country: req?.body?.about_country ? req?.body?.about_country : '',
        about_date: req?.body?.about_date ? req?.body?.about_date : '',
        about_alt_tag: req?.body?.about_alt_tag ? req?.body?.about_alt_tag : '',
        about_organiser: req?.body?.about_organiser ? req?.body?.about_organiser : '',
        about_venue: req?.body?.about_venue ? req?.body?.about_venue : '',
        about_companies: req?.body?.about_companies ? req?.body?.about_companies : '',
        mobile_no: req?.body?.mobile_no ? req?.body?.mobile_no : '',
        about_website: req?.body?.about_website ? req?.body?.about_website : '',
        about_image: req?.body?.about_image ? req?.body?.about_image : '',
        about_email: req?.body?.about_email ? req?.body?.about_email : '',
        about_status: req?.body?.about_status ? req?.body?.about_status : 0,
        site_id: req?.body?.site_id ? req?.body?.site_id : '',
      }
      var sql = "update about set about_name=?,about_select_sector=?,about_country=?,about_date=?,about_alt_tag=?,about_organiser=?,about_venue=?,about_companies=?,mobile_no=?,about_website=?,about_image=?,about_email=?,about_status=?,site_id=? where about_id=?"
      var data1 = Object.values(obj)
